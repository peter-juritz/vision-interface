...
#include <poll.h>
void
video_driver::errno_exit (const char *s)
{
...
*/
int
video_driver::xioctl (int fd, int request, void *arg)
{
  int r;
...
void
video_driver::init_read (unsigned int buffer_size)
{
...
void
video_driver::stop_capturing (void)
{
...
void
video_driver::close_device (void)
{
  if (-1 == close (fd))
    errno_exit ("close");
...
void
video_driver::start_capturing (void)
{
  unsigned int i;
  enum v4l2_buf_type type;
...
void
video_driver::uninit_device (void)
{
  unsigned int i;
