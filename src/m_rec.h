...
public:
  ...
  int bottom_left[2];
  int bottom_right[2];
  int num_ang_check, num_checks, num_pls;
  ...
  struct line old_j;
    ...
    m_rec (frame & f);
    frame & l_frame;
    std::vector < l_rec * >recs;
  bool orient ();
  void set_found (struct line *a, struct line *b, struct line *i,
		  struct line *j);
  bool find_monitor ();
  bool check_quadrangle (struct line *a, struct line *b, struct line *i,
			 struct line *j, struct screen_obstruction *scr_ob);
  bool check_mag (struct line *a, struct line *b, struct line *i,
		  struct line *j);
  ...
  bool border_check (int x1, int y1, int x2, int y2,
