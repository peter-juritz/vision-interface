...
void
rect (SDL_Surface * s, int x, int y, int w, int h, int r, int g, int b)
{
...
void
render (SDL_Surface * surface, int offx, int offy, frame & f)
{
  bool do_sync = false;
...
//              Uint32 cam_val = (Uint32) ( (Uint8 *) buffers[0].start)[y*288 + x];
	  ...
	  if (f.get_y (x, y) < f.y_thresh)
	    {
	      color =
		SDL_MapRGB (surface->format, f.get_y (x, y), f.get_y (x, y),
			    f.get_y (x, y));
	    }
	  else
	    {
	  ...
	  Uint8 *bufp;
	  bufp =
	    (Uint8 *) surface->pixels + (y + offy) * surface->pitch + (x +
								       offx) *
	    3;
	  if (SDL_BYTEORDER == SDL_LIL_ENDIAN)
	    {
...
void
draw_line (SDL_Surface * surface, int offx, int offy, float theta, float r,
	   int w, int h)
{
  //theta = -theta + M_PI;
...
void
draw_line2 (SDL_Surface * surface, int offx, int offy, float theta, float r,
	    int w, int h)
{
  //theta = -theta + M_PI;
  float m = -(cosf (theta) / sinf (theta));
  ...
  for (int x = 0; x < w; x++)
	...
	continue;
      bufp =
	(Uint8 *) surface->pixels + ((int) (m * x + c) +
				     offy) * surface->pitch + (x + offx) * 3;
      if (SDL_BYTEORDER == SDL_LIL_ENDIAN)
	{
	  bufp[0] = color;
	  bufp[1] = color >> 8;
...
void
draw_line3 (SDL_Surface * surface, int offx, int offy, float theta, float r,
	    int w, int h)
{
  //theta = -theta + M_PI;
  float m = -(cosf (theta) / sinf (theta));
...
void
accum_render (SDL_Surface * surface, int offx, int offy, frame & f,
	      int *accum, int m, m_rec & mon_rec)
{
  bool do_sync = false;
  if (m == 0)
    {
  ...
  for (int y = 0; y < f.numrho / 20; y++)
      ...
      for (int x = 0; x < f.numangle / 20; x++)
	{
	  ...
	  Uint8 *bufp;
	  bufp =
	    (Uint8 *) surface->pixels + (x + offy) * surface->pitch + (y +
								       offx) *
	    3;
	  if (SDL_BYTEORDER == SDL_LIL_ENDIAN)
	    {
  ...
  for (int i = 0; i < min (45, (int) f.line_accum.size ()); i++)
    {
      ...
      Uint8 *bufp;
      bufp =
	(Uint8 *) surface->pixels + (r_off + offy) * surface->pitch +
	(theta_off + offx) * 3;
      if (SDL_BYTEORDER == SDL_LIL_ENDIAN)
	{
	  bufp[0] = color;
...
void
sobel_render (SDL_Surface * surface, int offx, int offy, frame & f)
{
  bool do_sync = false;
...
void
sobel_render2 (SDL_Surface * surface, int offx, int offy, frame & f,
	       int *edge)
{
  bool do_sync = false;
  int *sob = f.op;		//f.compute_sobel(true);
...
void
show_lrec (SDL_Surface * surface, int offx, int offy, l_rec & lr, int t)
{
  if (lr.state == 0)
    return;
...
void
gdot (SDL_Surface * s, int x, int y, int size, int r, int g, int b)
{
  Uint32 color = SDL_MapRGB (s->format, r, g, b);
...
void
gdot2 (frame & f, int x, int y, int size)
{
  for (int tx = x - size; tx <= x + size; tx++)
    {
      for (int ty = y - size; ty <= y + size; ty++)
...
void
big_dot (SDL_Surface * s, int x, int y)
{
  Uint32 color = SDL_MapRGB (s->format, 0, 255, 0);
...
void
fill (Uint32 color, SDL_Surface * s, int w, int h)
{
  for (int y = 0; y < h; y++)
    {
      for (int x = 0; x < w; x++)
...
void
line (SDL_Surface * s, int sx, int sy, int ex, int ey)
{
  int np = 150;
  for (int i = 0; i < np; i++)
    {
...
void
screen_ren (int offx, int offy, SDL_Surface * s, m_rec & m, frame & f)
{
  ...
  max_height =
    max (m.bottom_left[1] - m.top_left[1],
	 m.bottom_right[1] - m.top_right[1]);
  max_width =
    max (m.top_right[0] - m.top_left[0],
	 m.bottom_right[0] - m.bottom_left[0]);
  max_height = 480;
  max_width = 768;
  for (int y = 0; y < max_height; y++)
      ...
      for (int x = 0; x < max_width; x++)
	{
	  ...
	  Uint32 color =
	    SDL_MapRGB (s->format, f.get_r (tx, ty), f.get_g (tx, ty),
			f.get_b (tx, ty));
	  Uint8 *bufp =
	    (Uint8 *) s->pixels + (offy + y) * s->pitch + (x + offx) * 3;
	  bufp[0] = color;
	  bufp[1] = color >> 8;
	  bufp[2] = color >> 16;
	}
...
void
draw_orientation (SDL_Surface * s, monitor & mon)
{
  int o_size = 50;
  int side_l = 400;
  int height_l = 200;
  ...
  rect (s, SURFACE_WIDTH - side_l, SURFACE_HEIGHT - o_size, side_l, o_size, 0,
	255, 0);
  rect (s, SURFACE_WIDTH - o_size, SURFACE_HEIGHT - height_l, o_size,
	height_l, 0, 255, 0);
}
...
main (int argc, char **argv)
{
  ...
  SDL_Event event;
  SDL_Surface *outs =
    SDL_SetVideoMode (mon.out_width, mon.out_height, 0,
		      SDL_HWSURFACE | SDL_DOUBLEBUF | SDL_HWPALETTE);
  SDL_Overlay *yuv =
    SDL_CreateYUVOverlay (f.width, f.height, SDL_YUY2_OVERLAY, outs);
  ...
  int grey = SDL_MapRGB (outs->format, 80, 80, 80);
  m_rec monitor_rec (f);
  h_detect hand (&f, &monitor_rec);
  double screen_corners[4][2] =
  ...
  while (!done)
      ...
      if (prod)
	{
	  ...
	  f.compute_canny ();
	  struct timeval cannytv;
	  gettimeofday (&cannytv, NULL);
	  //goto ren;
	  bool fnd = false;
	  bool fm = monitor_rec.find_monitor ();
	  struct timeval fmtv;
	  gettimeofday (&fmtv, NULL);
	  struct timeval handtv;
	  if (fm)
	    {
	      fnd = true;
	      ...
	      if (monitor_rec.hand_detected)
		{
		  //          printf("Got a Hand!\n");
		  //goto ren;
		  hand.hand_search (monitor_rec.handx, monitor_rec.handy,
				    f.op);
		  gettimeofday (&handtv, NULL);
		  gdot2 (f, monitor_rec.handx, monitor_rec.handy - 8, 2);
		  int realx, realy;
		  for (int i = 0; i < hand.fingertips.size (); i++)
		    {
		      monitor_rec.transform_xy (hand.fingertips[i]->x,
						hand.fingertips[i]->y, &realx,
						&realy);
	      ...
	      else
		{
	      ...
	      gdot2 (f, monitor_rec.ptl.x, monitor_rec.ptl.y, 9);
	      gdot2 (f, monitor_rec.ptr.x, monitor_rec.ptr.y, 9);
	      gdot2 (f, monitor_rec.pbl.x, monitor_rec.pbl.y, 9);
	      gdot2 (f, monitor_rec.pbr.x, monitor_rec.pbr.y, 9);
	      /* fill(grey,outs,mon.out_width,mon.out_height);
	         draw_line3 (outs,100,100, monitor_rec.old_a.theta,monitor_rec.old_a.r,f.width,f.height);
	         draw_line3 (outs,100,100, monitor_rec.old_b.theta,monitor_rec.old_b.r,f.width,f.height);
	         draw_line3 (outs,100,100, monitor_rec.old_i.theta,monitor_rec.old_i.r,f.width,f.height);
	         draw_line3 (outs,100,100, monitor_rec.old_j.theta,monitor_rec.old_j.r,f.width,f.height); */
	      //SDL_Flip(outs);
	    }
	  else
	    {
	  ...
	  //  printf("%i %i %i %i %i %i %i %i          %i %i %i %i %i %i %i %i\n",frmcnt,fnd,hand.fingertips.size(),monitor_rec.num_checks,monitor_rec.num_ang_check,monitor_rec.num_pls,tv.tv_sec,tv.tv_usec,        starttv.tv_sec,starttv.tv_usec, cannytv.tv_sec,cannytv.tv_usec, fmtv.tv_sec,fmtv.tv_usec,handtv.tv_sec,handtv.tv_usec);
