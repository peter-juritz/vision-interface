...
inline int
frame::get_off (int x, int y)
{
  return ((y * width + x) / 2) * 4;
}

bool
frame::score_func (float r1, float t1, float r2, float t2)
{
  float dr = r1 - r2;
//    float dt=t1-t2;
...
int
frame::get_normalised_edge (int x, int y)
{
...
void
frame::search_box (int x, int y, int w, bool init)
{
...
int *
frame::compute_sobel (int startx, int starty, int boxwidth, int boxheight,
		      bool initialise)
{				// init : 1 reinit, 0 dont init
  if (accum == NULL)
    {
  ...
  float s1[][3] = { {-1, -2, -1}, {0, 0, 0}, {1, 2, 1} };
  float s2[][3] = { {-1, 0, 1}, {-2, 0, 2}, {-1, 0, 1} };
  for (int y = heightclip (starty); y < heightclip (starty + boxheight);
       y += 1)
    {
      for (int x = widthclip (startx); x < widthclip (startx + boxwidth);
	   x += 1)
	{
	  int sum1 = 0;
	  int sum2 = 0;
...
//            printf("s12 = %i %i\n",sum1,sum2);
	  ...
	  for (int t = 0; t < 3; t++)
	    {
	      float theta =
		angle + ((float) (t - 1)) * (M_PI / (float) n_theta);
	      if (theta < 0)
		{
		  theta += M_PI;
		}
		  ...
		  for (int i = 0; i < line_accum.size (); i++)
		    {
		      ...
		      if (score_func (r, theta, l->r, l->theta))
			{	// same locality
			  found = true;
			  int bestr_off =
			    ((l->r + max_r) / (2 * max_r)) * n_r;
...
void
frame::set_r (int x, int y, char val)
{
  buffer[get_off (x, y)] = val;
}

void
frame::set_g (int x, int y, char val)
{
  buffer[get_off (x, y) + 1] = val;
}

void
frame::set_b (int x, int y, char val)
{
  buffer[get_off (x, y) + 2] = val;
}

bool
frame::in_bounds (int x, int y)
{
